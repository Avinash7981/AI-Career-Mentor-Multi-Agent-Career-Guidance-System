  ...
  for (const part of event.content.parts) {
    if (part.text && !part.thought) {
      ...
      (_d = content.parts) == null ? void 0 : _d.push({
        text: "[".concat(event.author, "] called tool `").concat(part.functionCall.name, "` with parameters: ").concat(safeStringify(
          part.functionCall.args
        ))
      });
    } else if (part.functionResponse) {
      (_e = content.parts) == null ? void 0 : _e.push({
        text: "[".concat(event.author, "] tool `").concat(part.functionResponse.name, "` returned result: ").concat(safeStringify(
...
function mergeFunctionResponseEvents(events) {
  ...
  const mergedEvent = createEvent(__spreadProps(__spreadValues({}, events[0]), {
